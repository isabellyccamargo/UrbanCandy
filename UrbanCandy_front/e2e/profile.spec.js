...
test.describe('Fluxo de Cadastro de Usuário', () => {
  ...
  test('Deve cadastrar um novo usuário com sucesso', async ({ page }) => {
    ...
    const viaCepPromise = page.waitForResponse(
      (response) => response.url().includes('viacep.com.br') && response.status() === 200
    );
  ...
  // FALHA
  test('Deve impedir o cadastro e validar as regras de campos incorretos (CPF, Senha e E-mail)', async ({
    page,
  }) => {
