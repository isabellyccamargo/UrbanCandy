...
export default function EmployeeOrdersDetails() {
    ...
    // Formata corretamente a URL da imagem vinda do backend
    ...
    useEffect(() => {
        ...
        try {
            ...
            setOrderData(JSON.parse(decodeURIComponent(rawOrder)));
        } catch (e) {
    ...
    useEffect(() => {
        ...
        Promise.all([getOrderItems(validOrderId), getOrderStatuses()])
    ...
    const formatOrderDate = (dateString?: string) => {
        ...
        if (date.toDateString() === now.toDateString()) return `Recebido hoje às ${time}`;
        return `Recebido em ${date.toLocaleDateString('pt-BR')} às ${time}`;
    };
    ...
    const handleAdvanceStatus = async () => {
        ...
        if (!validOrderId) {
            showMessage({ title: 'Erro', message: 'ID do pedido inválido.', type: 'error' });
            return;
        }

        if (!orderData || statuses.length === 0 || actionInfo.disabled) return;
        ...
        let nextStatus = currentIndex !== -1 && currentIndex < sortedStatuses.length - 1
    ...
    const styles = StyleSheet.create({
        ...
        centerContainer: { alignItems: 'center', marginVertical: space.md },
        ...
        spaceBetweenRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: 6 },
        ...
        actionsContainer: { flexDirection: 'row', gap: space.sm, marginTop: space.xs, marginBottom: space.md },
        ...
        btnAction: {
            ...
            alignItems: 'center',
            borderRadius: radius.pill,
            backgroundColor: actionInfo.disabled ? '#CCC' : statusStyle.text,
        },
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.card}>
                    <View style={styles.row}>
                        ...
                        <Text style={styles.infoText}>
                            {orderData?.people?.name || orderData?.user?.name || 'Cliente sem nome'}
                        </Text>
                    </View>
                                ...
                                <View style={[styles.row, { flex: 1 }]}>
                                    ...
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <Text style={styles.productPrice}>
                                            {price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} cada
                                        </Text>
                                    </View>
                                </View>
                                <Text style={styles.infoText}>
                ...
                <View style={styles.actionsContainer}>
                    ...
                    <Pressable
                        ...
                        disabled={updating || actionInfo.disabled}
                        onPress={handleAdvanceStatus}
                    >
                        {updating ? (
                            <ActivityIndicator size="small" color={colors.white} />
                        ) : (
                            <Text style={styles.btnTextAction}>{actionInfo.text}</Text>
                        )}
                    </Pressable>
                </View>
            </ScrollView>
