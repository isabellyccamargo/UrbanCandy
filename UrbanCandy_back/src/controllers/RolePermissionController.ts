...
class RolePermissionController {
  static async findAllRolePermissions(
    ...
    try {
      ...
      const result = await RolePermissionService.findAllRolePermissions(pageNumber, sizeNumber);
  ...
  static async findByIdRolePermission(
    ...
    try {
      ...
      if (!id_role_permission || Array.isArray(id_role_permission)) {
        throw new ApiException('INVALID_ID', 400, 'id_role_permission');
      }
      ...
      const rolePermission = await RolePermissionService.findByIdRolePermission(id);
  ...
  static async findByRole(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const permissions = await RolePermissionService.findByRole(id);
  ...
  static async findByPermission(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id_permission } = req.params;

      if (!id_permission || Array.isArray(id_permission)) {
        throw new ApiException('INVALID_ID', 400, 'id_permission');
      }
      ...
      const roles = await RolePermissionService.findByPermission(id);
  ...
  static async createRolePermission(
    ...
    try {
      ...
      if (!id_role || !id_permission) {
        throw new ApiException('REQUIRED_FIELD', 400, 'id_role/id_permission');
      }
      ...
      res.status(201).json({
        message: 'Permissão atribuída ao role com sucesso',
        id_role_permission: rolePermission.id_role_permission,
      });
    } catch (error) {
      next(error);
    }
  ...
  static async deleteRolePermission(
    ...
    try {
      ...
      res.status(200).json({
        message: 'Permissão removida do role com sucesso',
      });
    } catch (error) {
      next(error);
    }
