...
class UserRoleService {
    async findAllUserRoles(
        ...
        return await UserRoleRepository.findAllUserRoles(
            limit,
            offset
        );
    }
    ...
    async findByIdUserRole(id_user_role: number) {
        const userRole =
            await UserRoleRepository.findByIdUserRole(
                id_user_role
            );
    ...
    async findByUser(id_user: number) {
        return await UserRoleRepository.findByUser(
            id_user
        );
    }

    async findByRole(id_role: number) {
        return await UserRoleRepository.findByRole(
            id_role
        );
    }
    ...
    async createUserRole(
        ...
        const user =
            await UserRepository.findByIdUser(id_user);
        ...
        const userRoleExists =
            await UserRoleRepository.findByUserAndRole(
                id_user,
                id_role
            );
