...
class RolePermissionRepository {
  async findAllRolePermissions(limit: number, offset: number) {
  ...
  async findByIdRolePermission(id_role_permission: number) {
    return await RolePermission.findByPk(id_role_permission);
  }
  ...
  async createRolePermission(rolePermissionData: Partial<RolePermission>) {
    return await RolePermission.create(rolePermissionData);
  }

  async deleteRolePermission(id_role_permission: number) {
