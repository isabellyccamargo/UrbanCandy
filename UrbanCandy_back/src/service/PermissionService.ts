...
class PermissionService {
    ...
    async findByIdPermission(id_permission: number) {
        const permission =
            await PermissionRepository.findByIdPermission(
                id_permission
            );
    ...
    async createPermission(
        ...
        return await PermissionRepository.createPermission({
            ...
            description:
                permissionData.description?.trim() || null,
        });
    }
    ...
    async updatePermission(
        ...
        if (permissionData.name) {
            ...
            if (
                permissionExists &&
                permissionExists.id_permission !== id_permission
            ) {
                throw new ApiException(
                    'PERMISSION_ALREADY_EXISTS',
                    409,
        ...
        if (permissionData.description) {
            permissionData.description =
                permissionData.description.trim();
        }
        ...
        return await PermissionRepository.findByIdPermission(
            id_permission
        );
    }
    ...
    async deletePermission(id_permission: number) {
        ...
        await PermissionRepository.deletePermission(
            id_permission
        );
    }
}
