...
class OfferService {
  ...
  async findByIdOffer(id_offer: number) {
    ...
    if (!offer) {
      throw new ApiException('OFFER_NOT_FOUND', 404, id_offer);
    }

    return offer;
  }

  async createOffer(offer: Offers, productIds: number[]) {
    if (!offer.name_offer?.trim()) {
      throw new ApiException('INVALID_OFFER_NAME', 400);
    }

    if (offer.price_offer <= 0) {
      throw new ApiException('INVALID_OFFER_PRICE', 400);
    }

    if (!productIds || productIds.length < 1 || productIds.length > 2) {
      throw new ApiException('INVALID_OFFER_PRODUCTS', 400);
    }
    ...
    for (const id_product of productIds) {
      ...
      if (!product) {
        throw new ApiException('PRODUCT_NOT_FOUND', 404, id_product);
      }
    }

    const createdOffer = await OfferRepository.createOffer(offer);
    ...
    return await OfferRepository.findByIdOffer(createdOffer.id_offer);
  }

  async updateOffer(offer: Offers, productIds?: number[]) {
    ...
    if (productIds) {
      if (productIds.length < 1 || productIds.length > 2) {
        throw new ApiException('INVALID_OFFER_PRODUCTS', 400);
      }
    ...
    return await OfferRepository.findByIdOffer(offer.id_offer);
  }
