...
class TypeOfDeliveryService {
    ...
    private async verifyName(
        ...
        const exists =
            await TypeOfDeliveryRepository.findByName(type.name);

        if (
            exists &&
            exists.id_type_delivery !== id
        ) {
            throw new ApiException(
                'TYPE_OF_DELIVERY_ALREADY_EXISTS',
                409
            );
        }
    }
    ...
    async findAll(
        ...
        return TypeOfDeliveryRepository.findAll(
            size,
            (page - 1) * size
        );
    }
    ...
    async findById(id: number) {
        const type =
            await TypeOfDeliveryRepository.findById(id);

        if (!type) {
            throw new ApiException(
                'TYPE_OF_DELIVERY_NOT_FOUND',
                404,
                id
            );
        }
    ...
    async update(
        ...
        return TypeOfDeliveryRepository.update(
            id,
            type
        );
    }
